...
class FnCollectRecord(TypedDict):
    overload_enabled: bool
    overload_scopes: dict[str, dict[str, Any]]  # TODO: change to int key
...
class Fn(Generic[P, R]):
    ...
    def __init__(
        ...
        self.shape = shape
        self.shape_signature = inspect.Signature(
            list(inspect.Signature.from_callable(shape).parameters.values())[1:]
        )
        self.behavior = behavior

        self.overload_param_map = overload_param_map or {}
        self.overload_params = {
            i: k for k, v in self.overload_param_map.items() for i in v
        }
    ...
    def get_artifact_record(self, collector: BaseCollector) -> FnCollectRecord:
        return collector.artifacts.setdefault(
            FnImplement(self),
            {
    ...
    def collect(
        ...
        **overload_settings: Any,
        ...
        def wrapper(
            ...
            artifact = self.get_artifact_record(collector)
            if self.has_overload_capability:
                for fn_overload, params in self.overload_param_map.items():
                    scope = artifact["overload_scopes"].setdefault(
                        fn_overload.identity, {}
                    )
    ...
    def override(
        ...
        **kwargs: P1.kwargs,
        ...
        def wrapper(
            entity: Callable[Concatenate[Any, P2], R2]
        ) -> OverridePerformEntity[P2, R2]:
